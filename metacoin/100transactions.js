...
async function asyncFunction(instance,acc0,acc1,amount) {
    ...
    fs.appendFile("./../logs/heavyTransactionOutputTimestamp.txt", Date.now(), function(err) {
        if(err) {
            return console.log(err);
        }
    })

    fs.appendFile("./../logs/heavyTransactionOutputTimestamp.txt", "\n", function(err) {
        if(err) {
            return console.log(err);
        }
    ...
    fs.appendFile("./../logs/heavyTransactionOutput.json", JSON.stringify(transaction), function(err) {
        if(err) {
            return console.log(err);
        }
